...
function eval_function_call(toCall, parameterValues, environment) {
  ...
  switch (func[0]) {
    ...
    default:
      throw ExecutionError(`Can only call functions, but trying to call '${func[0]}'`)
  }
}
...
function eval_cell_function(func, parameterValues, environment, toCall) {
  ...
  if (parameterValues.length !== parameters.length)
    throw ExecutionError(`${parameterValues.length} arguments passed to function ${stringify(toCall)}, but it requires ${parameters.length} arguments.`)
