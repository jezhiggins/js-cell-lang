...
async function evaluate(expressions, environment = topLevelEnvironment()) {
  environment = await environment

  let result = ['none', null]

  for await (const expression of expressions)
    result = await eval_expression(expression, environment)

  return result
}

async function eval_expression(expression, environment) {
  if (!expression)
    return null
...
async function eval_operation(operation, operand1, operand2, environment) {
  const lhs = await eval_expression(operand1, environment)
  const rhs = await eval_expression(operand2, environment)
...
async function eval_assignment(symbol, expression, environment, targetEnvironment = environment) {
  const [, name] = symbol;
  const value = await eval_expression(expression, environment)
...
async function eval_function_call(toCall, parameterValues, environment) {
  const func = await eval_expression(toCall, environment)
...
async function eval_cell_function(func, parameterValues, environment, toCall) {
  ...
  for (let p = 0; p !== parameters.length; ++p)
    await eval_assignment(parameters[p], parameterValues[p], environment, functionEnvironment)

  return evaluate(funcBody, functionEnvironment)
}

async function eval_builtin_function(func, parameterValues, environment) {
  const functionEnvironment = environment.newScope()
  const parameters = await Promise.all(
    parameterValues.map(p => evaluate([p], functionEnvironment))
  )
